...
export default function ResultPage() {
  ...
  const {
    ...
    accuracy, rank, time_taken_seconds,
    wrong_questions   = [],
    skipped_questions = [],
    answers           = []
  } = result
  ...
  // answer map: question_id → selected_option
  const answerMap = {}
  answers.forEach(a => { answerMap[String(a.question_id)] = a.selected_option })

  function QuestionCard({ q, index, label, labelColor, labelBg, studentAns }) {
    return (
        ...
        {Object.entries(q.options).map(([key, val]) => {
          ...
          return (
            <div key={key} style={{
              ...
              {isCorrect  && <span style={{ fontSize: '0.72rem', color: 'var(--success)', marginLeft: '8px' }}>(सही उत्तर)</span>}
              {isSelected && !isCorrect && <span style={{ fontSize: '0.72rem', color: 'var(--error)', marginLeft: '8px' }}>(आपका उत्तर)</span>}
            </div>
          )
        })}
          ...
          <div className="card">
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              <div style={{ background: 'var(--border)', borderRadius: '8px', height: '12px', overflow: 'hidden' }}>
                <div style={{
                  ...
                  height: '100%', borderRadius: '8px', transition: 'width 1s'
                }} />
              </div>
            </div>
            ...
            {(wrong_questions.length > 0 || skipped_questions.length > 0) && (
              <div style={{ marginTop: '16px', padding: '12px', background: '#FFF3CD', borderRadius: '8px', fontSize: '0.85rem', color: '#856404' }}>
                💡 <strong>{wrong_questions.length} galat</strong> aur{' '}
                <strong>{skipped_questions.length} skip</strong> — review tab mein jaake padh lo!
              </div>
            )}
          </div>
        )}
        ...
        {tab === 'all' && (
          <div className="card">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(90px, 1fr))', gap: '8px' }}>
              {answers.map((a, i) => (
                <div key={i} style={{
                  padding: '8px', borderRadius: '8px', textAlign: 'center', fontSize: '0.8rem',
