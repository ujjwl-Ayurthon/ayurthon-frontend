...
export default function TestAttempt() {
  ...
  useEffect(function() {
    // STEP 1: Check localStorage device-level lock first
    var savedResultId = localStorage.getItem(ATTEMPT_KEY)
    if (savedResultId) {
      navigate('/result/' + savedResultId, {
        state: { alreadyAttempted: true, testTitle: 'This Test' }
      })
      return
    }
    ...
    api.get('/api/tests/attempt/' + token)
      .then(function(r) {
        ...
        // Check for existing session (tab closed mid-test)
      ...
      .catch(function() {
        setError('Test nahi mila ya abhi live nahi hai')
        setLoading(false)
      })
  }, [token])
  ...
  function saveSession(extraAnswers, extraQ) {
    var session = {
      name: name,
      tgUser: tgUser,
      answers: extraAnswers !== undefined ? extraAnswers : answersRef.current,
      currentQ: extraQ !== undefined ? extraQ : 0,
  ...
  async function handleRegister(e) {
    ...
    // Check backend if already attempted
    if (tg) {
      try {
  ...
  async function doSubmit(sName, sTg, currentAnswers, testData, startTime) {
    ...
    try {
      ...
      // Handle already submitted from backend
      if (res.data.alreadySubmitted) {
        localStorage.removeItem(SESSION_KEY)
        localStorage.setItem(ATTEMPT_KEY, String(res.data.resultId))
        navigate('/result/' + res.data.resultId, {
          state: { alreadyAttempted: true, testTitle: testData ? testData.title : '' }
        })
        return
      }

      // STEP 2: Lock device on successful submit
          ...
          <div className="card">
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '20px', textAlign: 'center' }}>
              <div style={{ background: 'var(--saffron-light)', borderRadius: '8px', padding: '12px' }}>
                <div style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--saffron)' }}>
            ...
            <form onSubmit={handleRegister}>
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  value={tgUser}
                  onChange={function(e) { setTgUser(e.target.value) }}
                  required
                />
                <small style={{ color: 'var(--text-muted)', fontSize: '0.78rem' }}>
                  ⚠️ Ek username se sirf ek baar attempt allowed hai
                </small>
              </div>
  ...
  var attemptedCount = Object.keys(answers).length
    ...
    <div style={{ minHeight: '100vh', background: '#F8F9FA' }}>
      <div className="cbt-header">
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{ fontSize: '0.9rem', color: 'var(--saffron)' }}>
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '20px 16px' }}>
        <div className="card" style={{ marginBottom: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
            <span className="badge" style={{ background: 'var(--saffron-light)', color: 'var(--saffron)' }}>
              Question {currentQ + 1}
            </span>
            <span className="badge" style={{ background: '#F0F4FF', color: '#3730A3' }}>
          ...
          {Object.entries(q.options).map(function(entry) {
            ...
            return (
              <button
                ...
                onClick={function() { setAnswer(q._id, key) }}
              >
          ...
          {answers[q._id] && (
            <button
              ...
              onClick={function() { clearAnswer(q._id) }}
            >
              ✕ Clear selection
            </button>
          )}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <button
            ...
            onClick={function() { handleQChange(Math.max(0, currentQ - 1)) }}
            disabled={currentQ === 0}
          >
            ← Prev
          </button>
          ...
          <button
            className="btn btn-outline"
            onClick={function() { handleQChange(Math.min(test.questions.length - 1, currentQ + 1)) }}
            ...
            {test.questions.map(function(_, i) {
              ...
              return (
                <button
                  ...
                  onClick={function() { handleQChange(i) }}
                >
                  {i + 1}
                </button>
              )
