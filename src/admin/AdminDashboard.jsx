...
function isAdminLoggedIn() {
  var t = getAdminToken();
  // token exists aur empty nahi hai — length check nahi karo
  return !!(t && t.length > 0);
}
...
function GlassCard(props) {
  return (
    <div style={Object.assign({ background: "rgba(255,255,255,0.72)", backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)", border: "1px solid rgba(255,255,255,0.62)", borderRadius: "18px", boxShadow: "0 8px 32px rgba(15,23,42,0.07)", padding: "24px" }, props.style || {})}>
      {props.children}
    </div>
  );
}
...
function ResetPasswordModal(props) {
    ...
    apiFetch("/api/students/" + student._id + "/reset-password", { method: "POST", headers: adminHeaders() })
      .then(function (r) {
        ...
        if (pw) { setResult(pw); }
          ...
          var msg = r.data.message || r.data.error || ("Error HTTP " + r.status);
          setError(msg);
        }
      });
  }
  function handleCopy() {
    if (!result) return;
    try { navigator.clipboard.writeText(result).then(function () { setCopied(true); setTimeout(function () { setCopied(false); }, 2000); }); }
    catch (e) { var el = document.createElement("textarea"); el.value = result; document.body.appendChild(el); el.select(); document.execCommand("copy"); document.body.removeChild(el); setCopied(true); setTimeout(function () { setCopied(false); }, 2000); }
  }
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.55)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "24px" }}>
      <div style={{ background: "rgba(255,255,255,0.96)", borderRadius: "22px", boxShadow: "0 32px 80px rgba(15,23,42,0.18)", padding: "32px", width: "100%", maxWidth: "400px", position: "relative" }}>
        ...
        <div style={{ marginBottom: "24px" }}>
          <div style={{ width: "48px", height: "48px", borderRadius: "14px", background: "rgba(212,175,55,0.12)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "14px" }}><RefreshCw size={22} color="#92700a" /></div>
          <h2 style={{ fontFamily: "Georgia,serif", fontSize: "18px", fontWeight: "700", color: "#0f172a", margin: "0 0 6px" }}>Reset Password</h2>
          <p style={{ fontSize: "13px", color: "#64748b", margin: "0" }}>Student: <strong>@{student.telegram_username}</strong> ({student.name || "—"})</p>
        </div>
        {!result ? (
          <>
            <div style={{ background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.22)", borderRadius: "12px", padding: "12px 16px", marginBottom: "24px" }}>
          ...
          <>
            <div style={{ textAlign: "center", marginBottom: "20px" }}>
              <CheckCircle size={32} color="#0D9488" style={{ marginBottom: "10px" }} />
              <p style={{ fontSize: "14px", color: "#475569", margin: "0" }}>Password reset ho gaya! Student ko share karo:</p>
            </div>
            <div style={{ background: "rgba(13,148,136,0.06)", border: "1.5px solid rgba(13,148,136,0.22)", borderRadius: "12px", padding: "16px", marginBottom: "12px", display: "flex", alignItems: "center", gap: "12px" }}>
              <span style={{ fontFamily: "monospace", fontSize: "18px", fontWeight: "700", color: "#0f172a", letterSpacing: "2px", flex: 1 }}>{showPass ? result : "•".repeat(result.length)}</span>
...
function StudentManagement() {
  ...
  function fetchStudents() {
    ...
    apiFetch("/api/students", { headers: adminHeaders() }).then(function (r) {
      setLoading(false);
      if (!r.ok) {
        var msg = r.status === 0 ? "Server unreachable. Refresh karo." : (r.data.message || "Error HTTP " + r.status);
        setError(msg); return;
      }
      setStudents(extractArray(r.data, ["students", "data", "result", "users"]));
    });
  }
  ...
  var th = { padding: "10px 14px", textAlign: "left", fontSize: "11px", fontWeight: "700", color: "#64748b", textTransform: "uppercase", borderBottom: "1px solid rgba(226,232,240,0.80)", background: "rgba(248,250,252,0.60)", whiteSpace: "nowrap" };
    ...
    <GlassCard style={{ padding: "0", overflow: "hidden" }}>
      <div style={{ padding: "20px 24px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "16px", flexWrap: "wrap", borderBottom: "1px solid rgba(226,232,240,0.60)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <div><div style={{ fontFamily: "Georgia,serif", fontSize: "16px", fontWeight: "700", color: "#0f172a" }}>Student Management</div><div style={{ fontSize: "11px", color: "#64748b" }}>{loading ? "Loading..." : students.length + " students"}</div></div>
        </div>
        <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
          ...
          <button onClick={fetchStudents} disabled={loading} style={{ display: "flex", alignItems: "center", gap: "6px", padding: "9px 14px", borderRadius: "10px", border: "1.5px solid rgba(13,148,136,0.22)", background: "rgba(13,148,136,0.07)", color: "#0D9488", fontSize: "13px", fontWeight: "600", cursor: "pointer" }}>
      ...
      {loading ? (
        ...
        <div style={{ padding: "40px", textAlign: "center" }}>
          ...
          <div style={{ padding: "12px 16px", borderRadius: "12px", marginBottom: "16px", background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.25)", color: "#92400e", fontSize: "13px" }}>{error}</div>
        ...
        <div style={{ padding: "56px", textAlign: "center", color: "#94a3b8", fontSize: "14px" }}>{search ? "Koi result nahi mila." : "Abhi tak koi student register nahi hua."}</div>
      ) : (
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "600px" }}>
            <thead><tr><th style={th}>#</th><th style={th}>Name</th><th style={th}>Username</th><th style={th}>Registered</th><th style={th}>Tests</th><th style={th}>Status</th><th style={Object.assign({}, th, { textAlign: "center" })}>Reset</th></tr></thead>
            <tbody>
              {filtered.map(function (s, idx) {
                ...
                return (
                  <tr key={s._id || idx} onMouseEnter={function (e) { e.currentTarget.style.background = "rgba(13,148,136,0.03)"; }} onMouseLeave={function (e) { e.currentTarget.style.background = "transparent"; }}>
                    <td style={Object.assign({}, td, { color: "#94a3b8", fontWeight: "600" })}>{idx + 1}</td>
                    <td style={td}><div style={{ display: "flex", alignItems: "center", gap: "10px" }}><div style={{ width: "32px", height: "32px", borderRadius: "50%", background: "linear-gradient(135deg,#0D9488,#D4AF37)", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: "700", fontSize: "13px" }}>{(s.name || "?").charAt(0).toUpperCase()}</div><span style={{ fontWeight: "600" }}>{s.name || "—"}</span></div></td>
                    ...
                    <td style={Object.assign({}, td, { textAlign: "center" })}><button onClick={function () { setResetSt(s); }} style={{ display: "inline-flex", alignItems: "center", gap: "6px", padding: "7px 12px", borderRadius: "8px", border: "1.5px solid rgba(212,175,55,0.30)", background: "rgba(212,175,55,0.08)", color: "#92700a", fontSize: "12px", fontWeight: "700", cursor: "pointer" }}><RefreshCw size={12} /> Reset</button></td>
                  </tr>
                );
              })}
            </tbody>
      ...
      {!loading && !error && filtered.length > 0 && <div style={{ padding: "12px 24px", borderTop: "1px solid rgba(226,232,240,0.60)", fontSize: "12px", color: "#94a3b8", textAlign: "right" }}>{filtered.length} / {students.length} shown</div>}
...
function AdminDashboard() {
  ...
  useEffect(function () {
    // ── SIMPLE auth guard — only localStorage, NO backend call on mount ──────
    // Backend call on mount causes loop when Render is cold starting
    if (!isAdminLoggedIn()) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(145deg,#F1F5F9 0%,#e8f4f1 40%,#F1F5F9 100%)", fontFamily: "'Inter','Segoe UI',system-ui,sans-serif" }}>
      <div style={{ position: "sticky", top: 0, zIndex: 50, background: "rgba(255,255,255,0.80)", backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)", borderBottom: "1px solid rgba(226,232,240,0.70)", padding: "0 24px" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: "60px", gap: "12px", flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            ...
            <div><div style={{ fontFamily: "Georgia,serif", fontWeight: "700", fontSize: "15px", color: "#0f172a" }}>AYURTHON</div><div style={{ fontSize: "10px", color: "#64748b" }}>ADMIN PANEL</div></div>
          </div>
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "28px 20px" }}>
        {activeTab === "overview" && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: "16px", marginBottom: "28px" }}>
              ...
              <StatCard label="MCQ Questions" value={stats ? (stats.mcq || 0) : 0} loading={statsLoading} icon={BookOpen} />
              <StatCard label="Platform Uptime" value="99.9%" loading={false} icon={TrendingUp} iconBg="rgba(34,197,94,0.10)" iconColor="#16a34a" />
            </div>
            <GlassCard style={{ padding: "0", overflow: "hidden" }}>
              <div style={{ padding: "18px 24px", borderBottom: "1px solid rgba(226,232,240,0.60)" }}><div style={{ fontFamily: "Georgia,serif", fontSize: "16px", fontWeight: "700", color: "#0f172a" }}>Live Tests</div></div>
              {liveTests.length === 0
                ? <div style={{ padding: "36px", textAlign: "center", color: "#94a3b8", fontSize: "14px" }}>Abhi koi test published nahi hai.</div>
                : <div style={{ padding: "8px 0" }}>{liveTests.map(function (test) { return (
                    <div key={test._id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 24px", borderBottom: "1px solid rgba(226,232,240,0.40)", gap: "12px", flexWrap: "wrap" }}>
                      <div><div style={{ fontWeight: "600", color: "#0f172a", fontSize: "14px" }}>{test.title}</div><div style={{ fontSize: "12px", color: "#64748b" }}>{Array.isArray(test.questions) ? test.questions.length : "?"} Qs · {test.duration_minutes} min</div></div>
                      <span style={{ padding: "4px 12px", borderRadius: "99px", fontSize: "11px", fontWeight: "700", background: "rgba(13,148,136,0.10)", color: "#0D9488", border: "1px solid rgba(13,148,136,0.22)" }}>🟢 Live</span>
                    </div>
                  ); })}</div>
              }
            </GlassCard>
        ...
        {activeTab === "students" && (
          <div>
            <div style={{ marginBottom: "24px" }}><h1 style={{ fontFamily: "Georgia,serif", fontSize: "22px", fontWeight: "700", color: "#0f172a", margin: "0 0 4px" }}>Student Management</h1><p style={{ fontSize: "13px", color: "#64748b", margin: "0" }}>Saare registered students.</p></div>
            <StudentManagement />
          </div>
        )}
      </div>
