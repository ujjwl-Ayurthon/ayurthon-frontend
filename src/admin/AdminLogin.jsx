...
import { BookOpen, Lock, Eye, EyeOff, Zap, ChevronRight } from "lucide-react";
    ...
    api.admin.login(password)
      .then(function(result) {
        ...
        if (result.ok) {
          ...
          var token = result.data.token || (result.data.data && result.data.data.token) || password;
          localStorage.setItem("admin_token", token);
          navigate("/admin/dashboard", { replace: true });
        } else {
          setError(result.data.message || "Galat password entered. Dobara try karo!");
        }
      })
      .catch(function() {
        setLoading(false);
        setError("Backend connectivity issue. Internet ya server check karo.");
      });
  }
